...
class tableRelModel(nn.Module):
    def __init__(self, config):
        super(tableRelModel, self).__init__()
        self.config = config
        self.bert_dim = config.bert_dim

        self.projection_matrix = nn.Linear(self.bert_dim * 2, self.bert_dim * 3)
        self.dropout = nn.Dropout(self.config.entity_pair_dropout)
        ...
        self.dropout_2 = nn.Dropout(self.config.dropout_prob)
    ...
    def triple_score_matrix(self, encoded_text, train=True):
        ...
        entity_pairs = self.dropout(entity_pairs)
    ...
    def forward(self, data, train=True):
        ...
        encoded_text = self.dropout_2(encoded_text)
        # [batch_size, tag_size, rel_num, seq_len, seq_len]
